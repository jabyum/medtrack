...
def check_medic_db(user_id):
    ...
    if checker:
        return True
    return False
def add_medic_db(user_id, name, speciality, experience, work_place_time, other, photo, messageid):
    db = next(get_db())
    new_medic = Medic(tg_id=user_id, name=name, speciality=speciality, experience=experience,
                      work_place_time=work_place_time, other=other, photo=photo,
                      messageid=messageid, reg_date=datetime.now())
    db.add(new_medic)
    db.commit()
    return new_medic.medic_id
...
def delete_exact_medic_db(user_id):
    db = next(get_db())
    exact_medic = db.query(Medic).filter_by(tg_id=user_id).first()
    if exact_medic:
        db.delete(exact_medic)
        db.commit()
        return exact_medic.messageid
